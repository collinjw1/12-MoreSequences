...
def run_test_generate_points_on_circle():
    ...
    # ------------------------------------------------------------------
    # Test 1:
    expected = [rg.Point(139.0, 53.0),  # All numbers are approximate.
                rg.Point(89.0, 103.0),
                rg.Point(39.0, 53.0),
                rg.Point(89.0, 3.0)]
    circle = rg.Circle(rg.Point(89, 53), 50)
...
def run_test_draw_points_on_circle():
    ...
    # ------------------------------------------------------------------
    title = 'DRAW_POINTS_ON_CIRCLE, test 4:  20 blue dots.'
    window = rg.RoseWindow(600, 600, title)
    circle = rg.Circle(rg.Point(300, 300), 125)
    circle.fill_color = 'green2'
...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    for k in range(number_of_points):
        loopcircle=rg.Circle(sequence[k],10)
        loopcircle.fill_color=color
        loopcircle.attach_to(window)
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    for k in range(number_of_slices):
        loopline=rg.Line(sequence[k],circle.center)
        loopline.color=color
        loopline.thickness=thickness
        loopline.attach_to(window)
...
def run_test_polygon():
    ...
    # ------------------------------------------------------------------
    title = 'POLYGON tests 1 and 2:  3 segments with thick blue lines;  6 with medium red lines.'
    window2 = rg.RoseWindow(600, 600, title)

    circle = rg.Circle(rg.Point(300, 300), 120)
    circle.outline_thickness = 3
    polygon(window2, circle, 8, 'green3', 3)
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    for k in range(number_of_segments-1):
        loopline=rg.Line(sequence[k],sequence[k+1])
        loopline.thickness=thickness
        loopline.color=color
        loopline.attach_to(window)

    lastline=rg.Line(sequence[number_of_segments-1],sequence[0])
...
def run_test_fancy_polygon():
    ...
    title = 'FANCY POLYGON test 4:  30 lime green lines on blue circle, hops = 13.'
    window = rg.RoseWindow(600, 600, title)

    circle = rg.Circle(rg.Point(300, 300), 150)
    circle.fill_color = 'yellow'
    fancy_polygon(window, circle, 300, 51, 'black', 3)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color, thickness):
    ...
    for k in range(number_of_lines):
        redrum=(redrum+hops_to_next_point) % number_of_lines
        loopline=rg.Line(sequence[shining],sequence[redrum])
        loopline.arrow='last'
        loopline.thickness=thickness
        loopline.color=color
        loopline.attach_to(window)
